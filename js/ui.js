...
function openWorkspaceFallback(panel) {
  ...
  if (workspace?.open) { workspace.open(panel); return true; }
  return false;
}
...
function bindButtons() {
  document.addEventListener("click", (event) => {
    ...
    if (teamButton) {
      ...
      if (targetTeam === "red" || targetTeam === "blue") document.dispatchEvent(new CustomEvent("codname:change-team", { detail: { team: targetTeam } }));
      return;
    }
    ...
    if (action === "show-profile") { if (!openWorkspaceFallback("profile")) focusSection(".account-panel", "پروفایل"); return; }
    if (action === "show-leaderboard") { if (!openWorkspaceFallback("leaderboard")) focusSection(".ranking-panel", "رتبه‌بندی"); return; }
    if (action === "show-decks") { if (!openWorkspaceFallback("decks")) focusSection(".lower-grid", "دسته‌های کارت"); return; }
    if (action === "show-settings") { if (!openWorkspaceFallback("settings")) showError("تنظیمات در دسترس نیست."); return; }
    if (action === "show-modes") { if (!openWorkspaceFallback("modes")) focusSection(".primary-actions-grid", "حالت‌های بازی"); return; }
  });
...
function focusSection(selector, label) {
  ...
  element.classList.remove("nav-focus"); void element.offsetWidth; element.classList.add("nav-focus"); showError(`${label} انتخاب شد.`);
}
...
function submitJoin() { const input = $("#join-code"); const code = String(input?.value || "").trim().toUpperCase(); if (!/^[A-Z0-9]{6}$/.test(code)) { showError("کد اتاق باید دقیقاً ۶ کاراکتر باشد."); input?.focus(); return false; } document.dispatchEvent(new CustomEvent("codname:join-room", { detail: { code } })); closeJoin(); return true; }
...
function bindAuth() {
  ...
  state.refs.authForm?.addEventListener("submit", async (event) => {
    event.preventDefault();
    const form = new FormData(event.currentTarget); const email = String(form.get("email") || "").trim(); const password = String(form.get("password") || ""); const username = String(form.get("username") || "").trim();
    setAuthBusy(true); setAuthStatus("در حال ارتباط با سرور...", false);
    try {
      if (state.authMode === "signup") {
        const data = await signUp({ email, password, username });
        if (!data.session) setAuthStatus("حساب ساخته شد. ایمیل تأیید را بررسی کن و سپس وارد شو.", false);
        else { setAuthStatus("حساب با موفقیت ساخته شد.", false); setTimeout(() => location.reload(), 400); }
      } else { await signIn({ email, password }); setAuthStatus("ورود موفق بود.", false); setTimeout(() => location.reload(), 300); }
    } catch (error) { console.error(error); setAuthStatus(authErrorMessage(error), true); }
    finally { setAuthBusy(false); }
  });
  $$('[data-auth-tab]').forEach((tab) => tab.addEventListener("click", () => switchAuthMode(tab.dataset.authTab)));
}
async function handleGoogleLogin() { setAuthBusy(true); setAuthStatus("در حال انتقال به Google...", false); try { await signInWithGoogle(); } catch (error) { console.error(error); setAuthStatus(authErrorMessage(error), true); setAuthBusy(false); } }
...
function renderAccount() { const n = $("[data-account-name]"), s = $("[data-account-status]"), l = $("[data-account-level]"), x = $("[data-account-xp]"), w = $("[data-account-wins]"); if (n) n.textContent = state.profile?.username || state.user?.email?.split("@")[0] || "مهمان"; if (s) s.textContent = state.user ? "عملیات آماده است" : "وارد نشده"; if (l) l.textContent = state.profile?.level ?? "—"; if (x) x.textContent = state.profile?.xp ?? "—"; if (w) w.textContent = state.profile?.games_won ?? "—"; }
...
function renderPlayers(players) { if (!state.refs.players) return; state.refs.players.innerHTML = ""; for (const player of players) { const isMe = player.user_id === state.user?.id; const team = player.team || "neutral"; const item = document.createElement("div"); item.className = `player-item team-${team}`; item.innerHTML = `<span>${player.last_seen_at ? "●" : "○"}</span><strong>${escapeHtml(player.display_name || "Player")}</strong><span class="team-label">${teamLabel(team)}</span>${player.is_host ? "<em>HOST</em>" : ""}${isMe && team !== "neutral" ? `<div class="team-switch" role="group" aria-label="تغییر تیم"><button type="button" class="team-btn team-btn-red ${team === "red" ? "is-selected" : ""}" data-team-switch="red">قرمز</button><button type="button" class="team-btn team-btn-blue ${team === "blue" ? "is-selected" : ""}" data-team-switch="blue">آبی</button></div>` : ""}`; state.refs.players.appendChild(item); } }
...
export function updateGameUI(game = {}) { renderBoard(game.board || [], game.revealed || [], game.currentTurn); renderPlayers(game.players || []); if (state.refs.clue) { state.refs.clue.textContent = ""; state.refs.clue.hidden = true; } const current = game.players?.find((p) => p.user_id === game.currentTurn); if (state.refs.turn) state.refs.turn.textContent = current ? `نوبت ${current.display_name}` : "منتظر بازیکن..."; if (state.refs.scoreRed) state.refs.scoreRed.textContent = String(game.scores?.red || 0); if (state.refs.scoreBlue) state.refs.scoreBlue.textContent = String(game.scores?.blue || 0); if (game.winnerTeam) showResult(game.winnerTeam, game.scores); }
function showResult(team, scores = {}) { const winner = $("[data-winner]"); const final = $("[data-final-score]"); if (winner) winner.textContent = team === "red" ? "تیم قرمز برنده شد" : "تیم آبی برنده شد"; if (final) final.textContent = `${scores.red || 0} - ${scores.blue || 0}`; }
