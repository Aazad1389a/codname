import { useHalloweenSpirit } from "./game.js";

// Temporary event window: exactly five days from 2026-09-14 09:17 (+03:30).
...
function renderCountdown() {
  document.querySelectorAll("[data-halloween-countdown]").forEach((el) => {
    if (!isHalloweenEventActive()) {
      el.textContent = "رویداد به پایان رسید";
      return;
    }
    el.textContent = `⏳ ${remainingText()} تا پایان رویداد`;
  });
}
...
function renderGamePower() {
  ...
  if (!game || game.status !== "playing" || !game.halloweenEvent || !isHalloweenEventActive()) {
    ...
    document.querySelectorAll("[data-halloween-event]").forEach((el) => el.remove());
    return;
  }
...
function renderMenuAndLobby() {
  ...
  targets.forEach((target) => {
    ...
    banner.innerHTML = `<b>🎃 ${HALLOWEEN_EVENT_NAME}</b><span>کدو، خفاش، شبح، جادوگر و ده‌ها کلمه هالووینی وارد بازی شده‌اند. هر بازیکن یک بار قدرت ویژه دارد.</span><em data-halloween-countdown>${remainingText()} تا پایان رویداد</em>`;
  });
}
...
function openPicker() {
  ...
  const own = (state.board || []).filter((card) => card.type === me.team && !revealed.has(Number(card.position)) && !(state.cards || []).some((matchCard) => Number(matchCard.position) === Number(card.position) && matchCard.revealed));
  ...
  list.querySelectorAll("[data-spirit-card]").forEach((button) => {
    button.addEventListener("click", async () => {
      ...
      try {
        const room = window.CODNAME?.getRoom?.();
        await useHalloweenSpirit({ roomId: room?.id, userId: me.user_id, cardId: button.dataset.spiritCard });
        ...
        if (toast) {
          toast.textContent = error?.message || "فعال‌سازی روح هالووینی انجام نشد.";
