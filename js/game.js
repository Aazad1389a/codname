...
function settingsFor(mode, overrides = {}) {
  return { ...(DEFAULTS[mode] || DEFAULTS.classic), ...overrides, mode };
}
...
function normalizeState(room, players = []) {
  ...
  return {
    ...
    roomId: room.id,
    status: room.status,
    mode: room.game_mode || state.mode || "classic",
    settings: room.settings || state.settings || {},
    currentTurn: room.current_turn ?? state.currentTurn ?? null,
    turnTeam: room.turn_team ?? state.turnTeam ?? "red",
    winnerTeam: room.winner_team ?? state.winnerTeam ?? null,
    players
  };
}
...
export async function createGame({ userId, mode = "classic", maxPlayers = 8, boardSize, playerFormat: format = "2v2" } = {}) {
  ...
  const normalizedMode = DEFAULTS[mode] ? mode : "classic";
  const normalizedFormat = format === "1v1" || normalizedMode === "duel" ? "1v1" : "2v2";
  const chosen = settingsFor(normalizedMode, {
  ...
  await databaseInsert("room_players", {
    room_id: room.id,
    user_id: userId,
    display_name: profile[0]?.username || "Player",
    team: "red",
    is_host: true
  });
...
export async function joinGame({ code, userId } = {}) {
  ...
  if (!existing.length) {
    ...
    if (players.length >= Number(room.max_players || 8)) throw new Error("Room is full.");
    const profile = await databaseSelect("profiles", { eq: { id: userId }, limit: 1 });
...
function teamCardsRemaining(board, revealed, team) {
  ...
  return (board || []).filter((card) => {
    ...
    if (revealedSet.has(position)) return false;
    if (card.type === team) return true;
    return card.type === CARD_TYPES.BONUS && card.bonusTeam === team;
  }).length;
}
...
async function finishMatch(state, winnerTeam, userId, nextScores, nextRevealed, powerState) {
  const finishedState = {
    ...
    phase: "finished",
    currentTurn: userId,
    turnTeam: teamForUser(state.players || [], userId) || state.turnTeam,
    winnerTeam,
  ...
  try {
    ...
    const { data: rewardData, error: rewardError } = await supabase.rpc("award_match_rewards", {
...
export async function startGame({ roomId, userId } = {}) {
  ...
  const players = await loadPlayers(roomId);
  const mode = DEFAULTS[room.game_mode] ? room.game_mode : "classic";
  const format = playerFormat(room.settings || {}, mode);
  const minimum = requiredPlayers(room.settings || {}, mode);
  ...
  if (format === "2v2" && (players.filter((p) => p.team === "red").length < 2 || players.filter((p) => p.team === "blue").length < 2)) throw new Error("در حالت 2 به 2 هر تیم باید حداقل ۲ بازیکن داشته باشد.");
  ...
  try {
    ...
    throw new Error("ساخت کارت‌های مسابقه انجام نشد. دوباره تلاش کن.");
  }
  ...
  const updatedRooms = await databaseUpdate("rooms", {
    status: "playing",
  ...
  if (!updatedRooms.length) throw new Error("این اتاق هم‌زمان توسط درخواست دیگری شروع شد. صفحه را تازه کن.");

  await databaseInsert("match_events", {
    match_id: match.id,
    actor_id: userId,
    event_type: "match_started",
    payload: { mode, boardSize: settings.boardSize, playerFormat: format, halloween }
  });
...
export async function selectCard({ roomId, userId, cardId } = {}) {
  ...
  const points = card.type === CARD_TYPES.BONUS
    ? (card.bonusTeam === team ? 2 : -2)
    : (card.type === CARD_TYPES.DARK ? -1 : (card.type === team ? 1 : 0));
  ...
  // Optimistic concurrency guard: only the first request is allowed to reveal this card.
  const revealedRows = await databaseUpdate("match_cards", {
    revealed: true,
    revealed_by: userId,
    reveal_value: points
  }, { match_id: state.matchId, position, revealed: false });
  ...
  await databaseInsert("match_events", {
    ...
    payload: {
      position,
      type: card.type,
      bonusTeam: card.type === CARD_TYPES.BONUS ? (card.bonusTeam || null) : null,
      points,
      source: "normal_turn"
    }
  });
  ...
  if (extraDraws > 0) {
    ...
    const nextState = {
      ...
      turnTeam: team,
      phase: "turn",
      bonusExtraDraws: nextExtraDraws,
      turnNumber: (state.turnNumber || 1) + 1
    };
    const updated = await databaseUpdate("rooms", { current_turn: userId, turn_team: team, game_state: nextState }, { id: roomId });
  ...
  const updated = await databaseUpdate("rooms", { current_turn: nextUser, turn_team: nextTeam, game_state: nextState }, { id: roomId });
  if (!updated.length) throw new Error("به‌روزرسانی نوبت انجام نشد.");
...
export async function endTurn({ roomId, userId } = {}) {
  ...
  if (remainingExtra > 0) throw new Error("به‌خاطر کارت طلایی هنوز یک انتخاب اضافه باقی مانده است.");
...
export async function useHalloweenSpirit({ roomId, userId, cardId } = {}) {
  ...
  const requested = cardId ? eligible.find((card) => card.id === cardId || String(card.position) === String(cardId)) : null;
  const chosen = requested || eligible[0];
  const result = await selectCard({ roomId, userId, cardId: chosen.id });
  const nextPower = { ...(state.halloweenPowerUsed || {}), [userId]: true };
  const latest = await getGameState(roomId);
  if (latest && latest.status === "playing") {
    const updated = await databaseUpdate("rooms", { game_state: { ...latest, halloweenPowerUsed: nextPower } }, { id: roomId });
    if (!updated.length) throw new Error("ثبت قدرت هالووین انجام نشد.");
  }
