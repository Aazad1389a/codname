...
export async function createGame({ userId, mode = "classic", maxPlayers = 8, boardSize } = {}) {
    ...
    const roomState = {
        board: [], turnNumber: 1, clue: null, guessesLeft: 0,
        scores: { red: 0, blue: 0 }, startedAt: null,
        phase: "lobby", clueGiver: null
    };
    const rooms = await databaseInsert("rooms", {
        code: roomCode, host_id: userId, status: "waiting", max_players: chosen.maxPlayers,
        turn_team: "red", game_state: roomState, game_mode: mode, settings: chosen
    });
    const room = rooms[0];
    ...
    await databaseInsert("room_players", {
        room_id: room.id, user_id: userId, display_name: displayName, team: "red", is_host: true
    });
...
export async function joinGame({ code, userId } = {}) {
    ...
    if (!existing.length) {
        ...
        await databaseInsert("room_players", {
            room_id: room.id, user_id: userId, display_name: displayName, team, is_host: false
        });
    }
...
function firstTeamPlayer(players, team) {
    return players.find((p) => p.team === team)?.user_id || null;
}

function chooseFirstTurn(players) {
    return firstTeamPlayer(players, "red") || firstTeamPlayer(players, "blue") || null;
}

function chooseClueGiver(players, team) {
    return firstTeamPlayer(players, team);
}

function chooseGuesser(players, team, clueGiver) {
    return players.find((p) => p.team === team && p.user_id !== clueGiver)?.user_id || null;
}
...
export async function startGame({ roomId, userId } = {}) {
    ...
    const matches = await databaseInsert("matches", {
        room_id: roomId, game_mode: mode, board_size: settings.boardSize,
        target_score: settings.targetScore, round_number: 1, started_at: new Date().toISOString()
    });
    ...
    await databaseInsert("match_cards", board.map((card, position) => ({
        match_id: match.id, position, card_id: null, card_type: card.type, revealed: false,
        reveal_value: card.type === CARD_TYPES.BONUS ? 2 : 1
    })));

    const clueGiver = chooseClueGiver(players, firstTeam);
    const nextState = {
        board, matchId: match.id, turnNumber: 1, clue: null, guessesLeft: 0,
        scores: { red: 0, blue: 0 }, startedAt: new Date().toISOString(),
        revealed: [], usedClues: [], phase: "clue", clueGiver
    };
    await databaseUpdate("rooms", {
        status: "playing", current_turn: clueGiver, turn_team: firstTeam,
        game_state: nextState, winner_team: null
    }, { id: roomId });
    await databaseInsert("match_events", {
        match_id: match.id, actor_id: userId, event_type: "match_started",
        payload: { mode, boardSize: settings.boardSize, clueGiver }
    });
...
export async function submitClue({ roomId, userId, text, number } = {}) {
    ...
    await databaseInsert("match_clues", {
        match_id: state.matchId, team: state.turnTeam, turn_number: state.turnNumber || 1,
        clue_text: clueText, clue_number: clueNumber, created_by: userId
    });
    const nextUser = chooseGuesser(state.players || [], state.turnTeam, userId);
    if (!nextUser) throw new Error("حداقل یک هم‌تیمی برای حدس‌زدن لازم است.");
    const nextState = {
        ...state,
        clue: { text: clueText, number: clueNumber, team: state.turnTeam, turnNumber: state.turnNumber || 1 },
        guessesLeft: clueNumber,
        phase: "guess",
        currentTurn: nextUser,
        clueGiver: userId,
        usedClues: [...(state.usedClues || []), clueText]
    };
    await databaseUpdate("rooms", {
        current_turn: nextUser, turn_team: state.turnTeam, game_state: nextState
    }, { id: roomId });
    await databaseInsert("match_events", {
        match_id: state.matchId, actor_id: userId, event_type: "clue_submitted",
        payload: { clue: clueText, number: clueNumber, team: state.turnTeam }
    });
...
export async function selectCard({ roomId, userId, cardId } = {}) {
    ...
    await databaseUpdate("match_cards", {
        revealed: true, revealed_by: userId, reveal_value: points
    }, { match_id: state.matchId, position });
    await databaseInsert("match_events", {
        match_id: state.matchId, actor_id: userId, event_type: "card_revealed",
    ...
    if (winner) {
        ...
        await databaseUpdate("rooms", {
            status: "finished", winner_team: winner, current_turn: userId,
            game_state: { ...state, scores: nextScores, revealed: nextRevealed, turnEnded: true, phase: "finished" }
        }, { id: roomId });
        await databaseUpdate("matches", {
            winner_team: winner, red_score: nextScores.red, blue_score: nextScores.blue,
    ...
    if (wrongCard || nextGuesses === 0) {
        ...
        const nextClueUser = nextClueGiver;
        const nextState = {
            ...state, scores: nextScores, revealed: nextRevealed,
            clue: null, guessesLeft: 0, phase: "clue",
            turnNumber: (state.turnNumber || 1) + 1,
            clueGiver: nextClueGiver
        };
        await databaseUpdate("rooms", {
            current_turn: nextClueUser, turn_team: nextTeam, game_state: nextState
        }, { id: roomId });
        return { turnEnded, gameFinished, gameState: await getGameState(roomId) };
    }
    ...
    const nextState = {
        ...state, scores: nextScores, revealed: nextRevealed,
        guessesLeft: nextGuesses, currentTurn: nextUser, phase: "guess"
    };
    await databaseUpdate("rooms", {
        current_turn: nextUser, turn_team: team, game_state: nextState
    }, { id: roomId });
    return { turnEnded, gameFinished, gameState: await getGameState(roomId) };
}
...
export async function endTurn({ roomId, userId } = {}) {
    ...
    const nextClueGiver = chooseClueGiver(players, nextTeam);
    const nextState = {
        ...state, clue: null, guessesLeft: 0, phase: "clue",
        turnNumber: (state.turnNumber || 1) + 1, clueGiver: nextClueGiver
    };
    await databaseUpdate("rooms", { current_turn: nextClueGiver, turn_team: nextTeam, game_state: nextState }, { id: roomId });
    await databaseInsert("match_events", {
        match_id: state.matchId, actor_id: userId, event_type: "turn_ended",
        payload: { nextTeam, nextUser: nextClueGiver }
    });
    return getGameState(roomId);
}
