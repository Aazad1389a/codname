...
const safe = (fn) => {
  try { return fn(); } catch (error) { console.error(error); return null; }
};
...
async function boot() {
  ...
  console.log(`CODNAME v${APP.version} ready`);
}

document.addEventListener("DOMContentLoaded", () => {
  boot().catch((error) => {
    console.error("CODNAME boot failed", error);
    showError("بازی نتوانست راه‌اندازی شود. تنظیمات Supabase را بررسی کنید.");
    showScreen("menu");
  });
});
...
function bindAppEvents() {
  document.addEventListener("codname:create-room", async (event) => {
    ...
    try {
      ...
      const result = await createGame({ userId: APP.user.id, ...(event.detail || {}) });
      APP.roomId = result.roomId;
      APP.roomCode = result.roomCode;
      await enterLobby();
    } catch (error) {
      console.error(error);
      showError(error?.message || "ساخت اتاق انجام نشد.");
      showScreen("menu");
    }
  });
  ...
  document.addEventListener("codname:join-room", async (event) => {
    ...
    try {
      ...
      const result = await joinGame({ userId: APP.user.id, code });
      APP.roomId = result.roomId;
      APP.roomCode = result.roomCode;
      await enterLobby();
    } catch (error) {
      console.error(error);
      showError(error?.message || "ورود به اتاق انجام نشد.");
      showScreen("menu");
    }
  });
  ...
  document.addEventListener("codname:start-game", async () => {
    ...
    try {
      showLoading("در حال شروع Match...");
      await startGame({ roomId: APP.roomId, userId: APP.user.id });
      await refreshGameState();
    } catch (error) {
      console.error(error);
      showError(error?.message || "شروع بازی انجام نشد.");
    }
  });
  ...
  document.addEventListener("codname:select-card", async (event) => {
    ...
    try {
      await selectCard({ roomId: APP.roomId, userId: APP.user.id, cardId: event.detail.cardId });
      await refreshGameState();
    } catch (error) {
      console.error(error);
      showError(error?.message || "انتخاب کارت انجام نشد.");
    }
  });
  ...
  document.addEventListener("codname:end-turn", async () => {
    if (!APP.user || !APP.roomId) return;
    try {
      await endTurn({ roomId: APP.roomId, userId: APP.user.id });
      await refreshGameState();
    } catch (error) {
      console.error(error);
      showError(error?.message || "پایان نوبت انجام نشد.");
    }
  });

  document.addEventListener("codname:leave-room", async () => {
    await leaveRoom();
    showScreen("menu");
  });

  document.addEventListener("codname:back-menu", async () => {
    await leaveRoom();
    showScreen("menu");
  });
}
...
async function setupRealtime() {
  if (APP.realtimeChannel) safe(() => unsubscribeFromRoom(APP.realtimeChannel));
  const connection = await connectRealtime();
  APP.realtimeChannel = await subscribeToRoom(connection, APP.roomId, () => refreshGameState().catch(console.error));
}

async function refreshGameState() {
  if (!APP.roomId) return;
  const state = await getGameState(APP.roomId);
  if (!state) return;
  APP.gameState = state;
  updatePlayerList(state.players || []);

  if (state.status === "waiting") {
    showLobby({ roomId: APP.roomId, roomCode: APP.roomCode, user: APP.user, profile: APP.profile, players: state.players || [], settings: state.settings || {} });
    showScreen("lobby");
  } else if (state.status === "playing") {
    showGame({ roomId: APP.roomId, roomCode: APP.roomCode, user: APP.user, profile: APP.profile, game: state });
    updateGameUI(state);
    showScreen("game");
...
async function leaveRoom() {
  if (APP.realtimeChannel) {
    await safe(() => unsubscribeFromRoom(APP.realtimeChannel));
...
if (supabase) {
  supabase.auth.onAuthStateChange(async () => {
    // OAuth redirects and email auth are handled with a clean reload so UI and profile state never race.
