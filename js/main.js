...
const APP={version:"1.2.0",user:null,profile:null,roomId:null,roomCode:null,gameState:null,realtimeChannel:null,initialized:false};
...
document.addEventListener("DOMContentLoaded",()=>boot().catch(error=>{console.error("CODNAME boot failed",error);showScreen("menu");setTimeout(()=>showError("بازی نتوانست راه‌اندازی شود. تنظیمات Supabase را بررسی کنید."),0)}));
...
if(supabase){
  supabase.auth.onAuthStateChange((event)=>{
    if(["SIGNED_IN","SIGNED_OUT","USER_UPDATED","PASSWORD_RECOVERY"].includes(event))window.setTimeout(()=>window.location.reload(),100);
  });
}
