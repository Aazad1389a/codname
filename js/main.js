...
const APP={version:"1.9.1",user:null,profile:null,roomId:null,roomCode:null,gameState:null,realtimeChannel:null,initialized:false};
...
function bindAppEvents(){
 ...
 document.addEventListener("codname:start-game",async()=>{if(!APP.user||!APP.roomId)return;try{showError("در حال ورود به بازی...");await startGame({roomId:APP.roomId,userId:APP.user.id});await refreshGameState()}catch(x){console.error(x);showError(x?.message||"شروع بازی انجام نشد.")}});
