...
const APP={version:"1.8.1",user:null,profile:null,roomId:null,roomCode:null,gameState:null,realtimeChannel:null,initialized:false};
...
function bindAppEvents(){
 ...
 document.addEventListener("codname:change-team",async e=>{if(!APP.user||!APP.roomId)return;const team=String(e.detail?.team||"");if(team!=="red"&&team!=="blue")return showError("تیم انتخاب‌شده معتبر نیست.");try{const rs=await getGameState(APP.roomId);if(!rs||rs.status!=="waiting")return showError("بعد از شروع بازی امکان تغییر تیم وجود ندارد.");const me=rs.players?.find(p=>p.user_id===APP.user.id);if(!me)return showError("بازیکن فعلی در این اتاق پیدا نشد.");if(me.team===team)return;await databaseUpdate("room_players",{team},{room_id:APP.roomId,user_id:APP.user.id});await refreshGameState()}catch(x){console.error(x);showError(x?.message||"تغییر تیم انجام نشد.")}});
