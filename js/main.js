...
async function loadAppModules(){
  // Dynamic imports prevent a slow mobile CDN/Supabase module from blocking the first paint.
  const [game,multiplayer,ui,player,supabase]=await Promise.all([
    import('./game.js'),
...
async function loadProfile(){
  ...
  try{
    ...
    if(!APP.profile){
      APP.profile=await withTimeout(api.player.createPlayerProfile(APP.user),5000,null);
    }
  }catch(error){
  ...
  try{
    api.ui.initUI({user:APP.user,profile:APP.profile});
  }catch(error){
    console.warn('CODNAME UI refresh failed:',error);
  }
}
...
function bindAppEvents(){
  ...
  document.addEventListener('codname:create-room',async event=>{
    ...
    try{
      ...
      console.error(error);
      api.ui.showError(error?.message||'ساخت اتاق انجام نشد.');
      api.ui.showScreen('menu');
    }
  });
  ...
  document.addEventListener('codname:join-room',async event=>{
    ...
    try{
      ...
      console.error(error);
      api.ui.showError(error?.message||'ورود به اتاق انجام نشد.');
      api.ui.showScreen('menu');
    }
  });
  ...
  document.addEventListener('codname:start-game',async()=>{
    ...
    try{
      api.ui.showLoading('در حال شروع Match...');
      const result=await withTimeout(api.game.startGame({roomId:APP.roomId,userId:APP.user.id}),10000,null);
      if(!result)console.warn('CODNAME start-game timed out');
      await refreshGameState();
    }catch(error){
      console.error(error);
      api.ui.showError(error?.message||'شروع بازی انجام نشد.');
  ...
  document.addEventListener('codname:select-card',async event=>{
    ...
    try{
      await withTimeout(api.game.selectCard({roomId:APP.roomId,userId:APP.user.id,cardId:event.detail.cardId}),10000,null);
      await refreshGameState();
  ...
  document.addEventListener('codname:end-turn',async()=>{
    if(!APP.user||!APP.roomId)return;
    try{
      await withTimeout(api.game.endTurn({roomId:APP.roomId,userId:APP.user.id}),10000,null);
      await refreshGameState();
...
async function setupRealtime(){
  if(APP.realtimeChannel){
    try{await api.multiplayer.unsubscribeFromRoom(APP.realtimeChannel);}catch(error){console.warn(error);}
    APP.realtimeChannel=null;
  }
  const connection=await withTimeout(api.multiplayer.connectRealtime(),5000,null);
  if(!connection)return;
  APP.realtimeChannel=await withTimeout(
    api.multiplayer.subscribeToRoom(connection,APP.roomId,()=>refreshGameState().catch(console.error)),
    5000,
    null
  );
}
...
async function refreshGameState(){
  ...
  if(!state)return;
  APP.gameState=state;
  api.ui.updatePlayerList(state.players||[]);

  if(state.status==='waiting'){
    api.ui.showLobby({roomId:APP.roomId,roomCode:APP.roomCode,user:APP.user,profile:APP.profile,players:state.players||[],settings:state.settings||{}});
    api.ui.showScreen('lobby');
  }else if(state.status==='playing'){
    api.ui.showGame({roomId:APP.roomId,roomCode:APP.roomCode,user:APP.user,profile:APP.profile,game:state});
    api.ui.updateGameUI(state);
    api.ui.showScreen('game');
  }else if(state.status==='finished'){
    api.ui.showGame({roomId:APP.roomId,roomCode:APP.roomCode,user:APP.user,profile:APP.profile,game:state});
    api.ui.updateGameUI(state);
    api.ui.showScreen('result');
  }
}

async function leaveRoom(){
  if(APP.realtimeChannel&&api?.multiplayer){
    try{await api.multiplayer.unsubscribeFromRoom(APP.realtimeChannel);}catch(error){console.warn(error);}
  }
  APP.realtimeChannel=null;
  APP.roomId=null;
...
async function boot(){
  // First paint: never wait for Supabase, Realtime, or another CDN.
  instantScreen('menu');

  try{
    await withTimeout(loadAppModules(),7000,null);
  }catch(error){
    console.error('CODNAME module load failed:',error);
    instantMessage('منوی بازی آماده است؛ اتصال آنلاین در پس‌زمینه در حال آماده‌شدن است.');
    return;
  }

  if(!api){
    instantMessage('اتصال آنلاین فعلاً در دسترس نیست.');
    return;
  }
  ...
  try{
    api.ui.initUI({user:APP.user,profile:null});
    api.ui.showScreen('menu');
  }catch(error){console.warn(error);}

  // User/profile loading never blocks the menu.
  void loadProfile();
