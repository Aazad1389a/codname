...
// 100 پایه × 100 پایه × 2 شکل = دقیقاً 20,000 کارت تک‌توکن.
const WORD_VARIANTS = Object.freeze(["", "یار"]);

export const WORD_CARDS = Object.freeze(
  WORD_VARIANTS.flatMap((variant) =>
    BASE_WORDS.flatMap((first) => BASE_WORDS.map((second) => `${first}${second}${variant}`))
  )
);
...
if (WORD_CARDS.length !== 20000 || WORD_CARDS.some((word) => /\s/.test(word))) {
  throw new Error(`CODNAME card pool must contain exactly 20000 single-word cards; got ${WORD_CARDS.length}.`);
}
