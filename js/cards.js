...
function makeOfflinePool() {
  return Object.freeze(Array.from({ length: POOL_SIZE }, (_, index) => CURATED_WORDS[index % CURATED_WORDS.length]));
}
...
function writeCachedPool(words) { try { localStorage.setItem(STORAGE_KEY, JSON.stringify(words)); } catch (_) {} }
...
async function loadRealPersianWords() {
  ...
  try {
    ...
    const response = await fetch(WORD_SOURCE_URL, { cache: "no-store", mode: "cors", signal: controller.signal }).finally(() => clearTimeout(timeout));
    ...
    for (const raw of [...CURATED_WORDS, ...sourceWords]) {
      ...
      seen.add(word); result.push(word);
...
export function createDeck(size = 25, event = "normal") {
  ...
  return randomize(uniqueSource).slice(0, safeSize).map((word, index) => ({ id: `local-card-${index}-${crypto.randomUUID()}`, word, position: index, type: CARD_TYPES.NEUTRAL, revealed: false, bonus: false, bonusTeam: null }));
}
...
export function createKey(size = 25, mode = "classic", firstTeam = "red") {
  ...
  if (mode === "expanded" || mode === "chaos") {
    ...
    for (let i = 0; i < bonusCount && cursor < positions.length; i += 1) {
      ...
      bonusTeams[position] = Math.random() < 0.5 ? teamA : teamB;
    }
  }
...
export function buildBoard(size = 25, mode = "classic", firstTeam = "red", event = "normal") {
  ...
  return deck.map((card, index) => ({ ...card, type: key[index], bonus: key[index] === CARD_TYPES.BONUS, bonusTeam: bonusTeams[index] }));
}
