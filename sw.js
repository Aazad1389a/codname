const CACHE = 'codname-shell-v11';
const ASSETS = [
  './', './index.html', './css/style.css', './css/menu-reference.css', './css/effects.css', './css/menu-visuals.css',
  './js/main.js', './js/ui.js', './js/ui-fixes.js', './js/install-entry.js', './js/github-auth-ui.js', './js/menu-visuals.js', './js/workspace.js', './js/workspace-v2.js', './js/auth.js', './js/game.js', './js/cards.js', './js/multiplayer.js', './js/player.js', './js/supabase.js',
...
self.addEventListener('fetch',event=>{
  ...
  if(isAppCode){event.respondWith(fetch(request).then(response=>{if(response.ok){const copy=response.clone();caches.open(CACHE).then(cache=>cache.put(request,copy));}return response;}).catch(()=>caches.match(request).then(cached=>cached||caches.match('./index.html'))));return;}
